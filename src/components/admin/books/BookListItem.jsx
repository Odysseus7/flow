...
class BookListItem extends Component {
	...
	editCourse = async (book) => {
		await admin
			.patch(`/books/${this.props.id}`, book)
			.then((response) => {
				successNotification("Changes are successfully saved");

				this.setState({ updatedTitle: book.name });
			})
			.catch((error) => {
				if (error["response"] && error["response"].status === 400) {
					errorNotification("Book not found");
					return;
				}
	...
	changeStatus = async (status) => {
		...
		await admin
			.patch(`/books/${this.props.id}?status=${status}`, null)
			.then((response) => {
				if (status === "active") {
					successNotification("Book has been restored");
				} else {
					successNotification("Book has been deleted");
				}
